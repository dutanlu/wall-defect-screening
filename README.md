...
**赛道定位**：本作品**不归属任一推荐赛道**，但理念上与赛道五「可解释性与人机交互」最接近（赛题对赛道方向为「推荐」而非「必选」，故不归属不扣分）。
...
│   ├── 外墙缺陷智能筛查_实机运行视频.mp4      提交物③ 实机运行视频（2:18 / 23.07 MB）
...
| `step5_fetch_rebar.py` | 补充数据源（露筋） | label-studio → YOLO 转换；**产物经 HRCDS 并入 V3 主数据集**（462 图 / 1389 框，见 `技术报告.md §4.1`） |
...
> **V3 起 7 类全部有数据、模型全部输出**（见上表逐类框数与 `01_data/dataset/wall_defects.yaml`
> 的 7 个类别名）。早期 V1/V2 只有 4 类，该限制**已于 V3 解除**；
> 但 `delamination` / `moss` 两类样本量仍偏少（694 / 560 框），
> 引用其**逐类指标**时应注明。这符合本项目「筛查工具而非鉴定工具」的定位。
...
# 5) 补充数据源：露筋（HRCDS）。**脚本只下载+转 YOLO 到隔离目录**，
#    不自动改主数据集；本项目的 V3 数据集是在 step5 之后**人工并入**的
#    （见技术报告 §4）：`exposed_rebar` 1389 框 / 462 图，train/val/test 均有。
...
| `v8n640` | yolov8n | 640 | 轻量对照（**本项目不参赛道二**，仅存档 <10MB 规格下的精度损失） |
...
> **口径声明**：本项目**不参赛道二（极端资源约束部署）**，主战场是「现场筛查的可判读性」。
> 本节提供的是**技术储备与诚实的失败记录** —— 量化后虽满足 <10MB 的体积规格，
> 但端到端检出质量下降，无法在不牺牲筛查能力的前提下声称达标。
> 详见 `07_report/技术报告.md §8.2`。
...
| 内容 | 是否入库 | 说明 |
|---|---|---|
| `01_data/` 数据集 | ❌ | 训练数据来自 6 个第三方公开数据集，各自许可**不允许我们代为再分发**；仓库只提供构建脚本 `02_code/step0*.py` ~ `step5_*.py` |
| `03_weights/` 模型权重 | ❌ | 体积较大；需要时按下方步骤自行训练，或另附 Release 附件 |
| `04_results/{train,ablation,vis}` | ❌ | 训练与实验大产物（合计约 900 MB） |
| `04_results/eval/` 评估指标 | ✅ | 报告中**所有引用数字的唯一来源**，体积小、可复核 |
| `logs/` 训练进度日志 | ❌ | 含逐帧 ANSI 进度条（单文件可达 7 MB），无信息价值；只保留记录结论与踩坑的 `.md` |

### 训练数据来源与许可

| 数据集 | 用途 | 许可 | 获取方式 |
|---|---|---|---|
| `SDNET2018` | 基础裂缝 | 见原始出处 | 公开 |
| `jiange1236/StructuralCrackDataset` | 露筋补充（实贡献 79 框 / 25 图） | Apache-2.0 | ModelScope 国内镜像 |
| `HRCDS` | ★ 补 `exposed_rebar` | CC BY 4.0 | Mendeley Data |
| `sanjuchinni/urban-infrastructure-anomalies` | ★ 补 `rust` / `delamination` | Apache-2.0 | Kaggle 公开 API（匿名可直下） |
| `MBDD2025` | 备选（已下载，**未并入**） | CC BY 4.0 | Kaggle |
| `BFDD` | 空鼓（已下载，**未并入**） | CC BY 4.0 | Mendeley Data |

详表与「为什么有些源被排除」的逐条论证见 `07_report/技术报告.md` 第 6 章。

### 复现步骤

见上文「快速开始 → 数据流水线」。核心两步：

1. 按 `step0` ~ `step5` 重建数据集（脚本会自动校验与去重）；
2. 跑 `train.py` 训练、`evaluate.py` 在独立测试集上评估。

> ⚠️ **口径纪律**：`evaluate.py` 在**独立测试集**上的结果是报告中数字的唯一合法来源。
> 训练过程打印的 mAP 是 val 集上的，**不得**引用为最终指标。
> 引用总体指标时必须同时给出逐类数字。

### 引用

如本工作对你的研究有帮助，欢迎引用本仓库。若需引用具体数字，
请以 `04_results/eval/` 下的 JSON 为准，并注明取数口径（测试集 / val 集）。
